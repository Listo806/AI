...
export class ApiAccessController {
  ...
  async get(@CurrentUser() user: any) {
    return this.apiAccessService.getApiKey(this.requireTeam(user));
  }

  @Get("usage")
  async usage(@CurrentUser() user: any) {
    return this.apiAccessService.getUsage(this.requireTeam(user));
  }

  @Post("generate")
  async generate(@CurrentUser() user: any) {
    return this.apiAccessService.generateApiKey(this.requireTeam(user));
  }

  @Post("revoke")
  async revoke(@CurrentUser() user: any) {
    return this.apiAccessService.revoke(this.requireTeam(user));
  }

  @Get("status")
  async status(@CurrentUser() user: any) {
    const apiKey = await this.apiAccessService.getApiKey(
