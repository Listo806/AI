...
export class PaymentsService {
  ...
  private webSolutionsTableReady = false;

  private async ensureWebSolutionsTable(): Promise<void> {
    if (this.webSolutionsTableReady) return;
  ...
  private resolveWebSolutionPrice(data: any):
    ...
    | null {
    const ids = this.extractPaddlePriceIds(data);

    const mapping = [
      {
        env: process.env.PADDLE_PRICE_WEB_CONNECTION,
        serviceId: 'connection-setup',
        serviceName: 'Connection Setup',
        expectedAmount: 147,
      },
      {
        env: process.env.PADDLE_PRICE_WEB_OPTIMIZATION,
        serviceId: 'website-optimization',
        serviceName: 'Website Optimization',
        expectedAmount: 297,
      },
      {
        env: process.env.PADDLE_PRICE_WEB_TRANSFORMATION,
    ...
    return (
      mapping.find(
        (item) =>
          !!item.env &&
          ids.includes(String(item.env).trim()),
      ) || null
    );
  }

  private async recordWebSolutionPurchase(
    data: any,
    service: {
      serviceId: string;
      serviceName: string;
    ...
    await this.ensureWebSolutionsTable();
    ...
    if (customUserId) {
      try {
        const { rows } = await this.db.query(
          `SELECT COALESCE(
             u.team_id,
             (SELECT t.id FROM teams t WHERE t.owner_id = u.id LIMIT 1)
           ) AS team_id
           FROM users u
           WHERE u.id = $1
           LIMIT 1`,
    ...
    await this.db.query(
      `
      INSERT INTO web_solution_orders (
      ...
      [
        ...
        data?.customer?.email ||
          custom?.email ||
          null,
        custom?.fullName || null,
        custom?.businessName || null,
        custom?.phone || null,
        custom?.website || null,
    ...
    this.logger.log(
      `Web Solutions purchase recorded: ${service.serviceId} / ${transactionId}`,
    );
  ...
  async processPaddleWebhook(body: any): Promise<{
    ...
    // --- Web Solutions one-time professional service purchase ---
    // Detect only by REAL server-side Paddle price ids. Client custom_data never
    // decides the amount or service tier.
    if (
      ...
      if (webSolution) {
        ...
        try {
          webMatched = await this.recordWebSolutionPurchase(
            data,
            webSolution,
            customUserId,
          );
          ...
          this.logger.error(
            `Web Solutions purchase ${data?.id || 'unknown'} failed: ${err?.message}`,
          );

          // This is a confirmed payment. Throw so Paddle retries the signed
          // webhook until the order is persisted.
          throw err;
        }
