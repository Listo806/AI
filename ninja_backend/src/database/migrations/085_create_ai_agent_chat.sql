CREATE TABLE IF NOT EXISTS public.ai_agent_chat_sessions (
    ...
    CONSTRAINT ai_agent_chat_sessions_status_check
      CHECK (
        status IN (
          'active',
          'archived'
...
CREATE TABLE IF NOT EXISTS public.ai_agent_chat_messages (
    ...
    session_id uuid NOT NULL
      REFERENCES public.ai_agent_chat_sessions(id)
      ON DELETE CASCADE,
    ...
    CONSTRAINT ai_agent_chat_messages_role_check
      CHECK (
        role IN (
          'user',
          'assistant',
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_team_updated
