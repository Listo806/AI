...
export class AestheticWellnessService {
  ...
  private requireTeam(teamId?: string | null) {
    if (!teamId) throw new ForbiddenException('A CRM team/account is required.');
    return teamId;
  }
  ...
  private async assertProvider(teamId: string, providerId?: string) {
    ...
    if (!rows[0]) throw new BadRequestException('Invalid provider for this workspace.');
    return rows[0];
  }
  ...
  private async assertLocation(teamId: string, locationId?: string) {
    ...
    if (!rows[0]) throw new BadRequestException('Invalid location for this workspace.');
    return rows[0];
  }
  ...
  async getSetupStatus(teamId: string) {
    ...
    const firstIncompleteStep = firstIncompleteIndex === -1 ? 7 : firstIncompleteIndex + 1;
  ...
  async getDashboard(teamId: string, filters: DashboardFilters = {}) {
    ...
    const todayApptWhere = [...apptWhere, this.rangeSql('today', 'a.start_at')].join(' AND ');
    const selectedApptWhere = [...apptWhere, this.rangeSql(filters.range || 'today', 'a.start_at')].join(' AND ');
    const todayLeadWhere = [...leadWhere, this.rangeSql('today', 'l.created_at')].join(' AND ');
    ...
    const [
      ...
      this.db.query(`SELECT COUNT(*)::int total FROM leads l WHERE ${todayLeadWhere}`, leadParams),
      this.db.query(
      ...
      this.db.query(`SELECT COUNT(*)::int total FROM appointments a WHERE ${todayApptWhere}`, apptParams),
      this.db.query(
        `SELECT COUNT(*)::int total FROM appointments a
      ...
      this.db.query(
        ...
        filters.providerId ? [teamId, WORKSPACE_ID, filters.providerId] : [teamId, WORKSPACE_ID],
      ),
      this.db.query(
        `SELECT COALESCE(SUM(r.revenue),0)::numeric revenue
           FROM aesthetic_treatment_records r
    ...
    const showNow = percent(Number(show.attended || 0), Number(show.eligible || 0));
    const showPrev = percent(Number(prevShow.attended || 0), Number(prevShow.eligible || 0));

    const treatmentRows = treatmentInterest.rows;
    const treatmentTotal = treatmentRows.reduce((sum: number, row: any) => sum + Number(row.total || 0), 0);
    ...
    return {
      ...
      metrics: {
        newInquiries: {
          ...
          trendPercent: trend(Number(newInquiries.rows[0]?.total || 0), Number(prevInquiries.rows[0]?.total || 0)),
        },
        consultationsBooked: {
          value: Number(bookedToday.rows[0]?.total || 0),
          trendPercent: trend(Number(bookedToday.rows[0]?.total || 0), Number(prevBooked.rows[0]?.total || 0)),
        },
        todaysAppointments: {
          value: Number(appointmentsToday.rows[0]?.total || 0),
          trendPercent: trend(Number(appointmentsToday.rows[0]?.total || 0), Number(prevAppointments.rows[0]?.total || 0)),
        },
        showRate: { value: showNow, trendPercent: showNow - showPrev },
        revenueToday: {
          value: Number(revenueToday.rows[0]?.revenue || 0),
          trendPercent: trend(Number(revenueToday.rows[0]?.revenue || 0), Number(prevRevenue.rows[0]?.revenue || 0)),
        },
        rebookingDue: {
          value: Number(rebookingDue.rows[0]?.total || 0),
          trendPercent: trend(Number(rebookingDue.rows[0]?.total || 0), Number(prevRebooking.rows[0]?.total || 0)),
        },
      },
      schedule: schedule.rows.map((r: any) => ({
        id: r.id,
        timeLabel: new Date(r.start_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }),
      ...
      treatmentInterest: treatmentRows.map((r: any) => ({
        name: r.name,
        percent: treatmentTotal ? Math.round((Number(r.total || 0) / treatmentTotal) * 100) : 0,
      })),
      rebookingClients: rebookClients.rows.map((r: any) => this.mapRebookingRow(r)),
  ...
  private async getConversationStats(teamId: string) {
    const { rows } = await this.db.query(
      `WITH clinic_messages AS (
       ...
       SELECT
         (SELECT COUNT(DISTINCT conversation_id)::int FROM clinic_messages) conversations,
         COALESCE(ROUND(AVG(seconds))::int, 0) avg_seconds
       FROM pairs
  ...
  private async getPipeline(teamId: string, providerId?: string) {
    const provider = providerId ? await this.assertProvider(teamId, providerId) : null;
    const leadFilter = provider?.user_id ? 'AND l.assigned_to = $3' : '';
    const leadParams = provider?.user_id ? [teamId, WORKSPACE_ID, provider.user_id] : [teamId, WORKSPACE_ID];
    const apptFilter = providerId ? 'AND a.aesthetic_provider_id = $3' : '';
    const apptParams = providerId ? [teamId, WORKSPACE_ID, providerId] : [teamId, WORKSPACE_ID];

    const [newInquiry, qualified, booked, confirmed, completed, followup] = await Promise.all([
      this.db.query(`SELECT COUNT(*)::int total FROM leads l WHERE l.team_id=$1 AND l.workspace_id=$2 AND l.status='new' ${leadFilter}`, leadParams),
      this.db.query(`SELECT COUNT(*)::int total FROM leads l WHERE l.team_id=$1 AND l.workspace_id=$2 AND (l.status='qualified' OR l.lead_ai_state='qualified') ${leadFilter}`, leadParams),
      this.db.query(`SELECT COUNT(*)::int total FROM appointments a WHERE a.team_id=$1 AND a.workspace_id=$2 AND a.type='consultation' AND a.status IN ('pending','confirmed','completed') ${apptFilter}`, apptParams),
      this.db.query(`SELECT COUNT(*)::int total FROM appointments a WHERE a.team_id=$1 AND a.workspace_id=$2 AND a.status='confirmed' ${apptFilter}`, apptParams),
      this.db.query(`SELECT COUNT(*)::int total FROM aesthetic_treatment_records r WHERE r.team_id=$1 AND r.workspace_id=$2 AND r.status='completed' ${providerId ? 'AND r.provider_id=$3' : ''}`, providerId ? [teamId, WORKSPACE_ID, providerId] : [teamId, WORKSPACE_ID]),
      this.db.query(`SELECT COUNT(*)::int total FROM lead_tasks t WHERE t.team_id=$1 AND t.workspace_id=$2 AND t.status IN ('pending','in_progress') ${provider?.user_id ? 'AND t.assigned_to=$3' : ''}`, provider?.user_id ? [teamId, WORKSPACE_ID, provider.user_id] : [teamId, WORKSPACE_ID]),
    ]);

    return [
      { id: 'new-inquiry', name: 'New Inquiry', count: Number(newInquiry.rows[0]?.total || 0) },
      { id: 'ai-qualified', name: 'AI Qualified', count: Number(qualified.rows[0]?.total || 0) },
      { id: 'consultation-booked', name: 'Consultation Booked', count: Number(booked.rows[0]?.total || 0) },
      { id: 'confirmed', name: 'Confirmed', count: Number(confirmed.rows[0]?.total || 0) },
      { id: 'treatment-completed', name: 'Treatment Completed', count: Number(completed.rows[0]?.total || 0) },
      { id: 'follow-up', name: 'Follow-Up', count: Number(followup.rows[0]?.total || 0) },
    ];
  }
  ...
  private async getFollowUpQueue(teamId: string, providerId?: string) {
    ...
    const [postTreatment, missed, unanswered, rebooking] = await Promise.all([
      this.db.query(`SELECT COUNT(*)::int total FROM aesthetic_treatment_records r WHERE r.team_id=$1 AND r.workspace_id=$2 AND r.status='completed' AND r.completed_at >= CURRENT_DATE - INTERVAL '7 days' ${providerId ? 'AND r.provider_id=$3' : ''}`, providerId ? [teamId, WORKSPACE_ID, providerId] : [teamId, WORKSPACE_ID]),
      this.db.query(`SELECT COUNT(*)::int total FROM appointments a WHERE a.team_id=$1 AND a.workspace_id=$2 AND a.status='canceled' AND a.start_at >= CURRENT_DATE - INTERVAL '30 days' ${providerId ? 'AND a.aesthetic_provider_id=$3' : ''}`, providerId ? [teamId, WORKSPACE_ID, providerId] : [teamId, WORKSPACE_ID]),
      this.db.query(`SELECT COUNT(*)::int total FROM lead_tasks t WHERE t.team_id=$1 AND t.workspace_id=$2 AND t.status IN ('pending','in_progress') AND (LOWER(t.title) LIKE '%unanswered%' OR LOWER(COALESCE(t.description,'')) LIKE '%unanswered%') ${assigned}`, params),
      this.db.query(`SELECT COUNT(*)::int total FROM aesthetic_treatment_records r WHERE r.team_id=$1 AND r.workspace_id=$2 AND r.next_rebook_at <= CURRENT_DATE + INTERVAL '7 days' AND r.next_rebook_at IS NOT NULL ${providerId ? 'AND r.provider_id=$3' : ''}`, providerId ? [teamId, WORKSPACE_ID, providerId] : [teamId, WORKSPACE_ID]),
    ]);

    return [
      { id: 'post-treatment', label: 'post-treatment check-ins', count: Number(postTreatment.rows[0]?.total || 0) },
      { id: 'missed-consultations', label: 'missed consultations', count: Number(missed.rows[0]?.total || 0) },
      { id: 'unanswered-inquiries', label: 'unanswered inquiries', count: Number(unanswered.rows[0]?.total || 0) },
      { id: 'rebooking-reminders', label: 'rebooking reminders', count: Number(rebooking.rows[0]?.total || 0) },
    ];
  }
  ...
  private async getProviderPerformance(teamId: string, locationId?: string) {
    ...
    return rows.map((r: any) => ({
      ...
      showRate: Number(r.eligible || 0) > 0 ? Math.round((Number(r.attended || 0) / Number(r.eligible)) * 100) : 0,
    }));
  }

  private async getRevenueOverview(teamId: string, providerId?: string) {
    const params = providerId ? [teamId, WORKSPACE_ID, providerId] : [teamId, WORKSPACE_ID];
    ...
    const changePercent = previous ? Math.round(((total - previous) / previous) * 100) : total ? 100 : 0;
    return {
      total,
      changePercent,
      points: rows.map((r: any) => ({
        label: new Date(`${r.day}T12:00:00Z`).toLocaleDateString('en-US', { weekday: 'short' }),
  ...
  private mapRebookingRow(r: any) {
    ...
    return {
      ...
      overdue: days < 0,
      dueLabel: days === 0 ? 'Due today' : days < 0 ? `${Math.abs(days)} day${Math.abs(days) === 1 ? '' : 's'} overdue` : `Due in ${days} day${days === 1 ? '' : 's'}`,
    };
  }
  ...
  async saveLocation(teamId: string, body: any, id?: string) {
    this.requireTeam(teamId);
    if (!String(body?.name || '').trim()) throw new BadRequestException('Location name is required.');
    const params = [teamId, WORKSPACE_ID, String(body.name).trim(), body.addressLine1 || null, body.addressLine2 || null, body.city || null, body.state || null, body.postalCode || null, body.country || null, body.timezone || 'UTC', body.isActive !== false];
    if (id) {
      const { rows } = await this.db.query(
  ...
  async listProviders(teamId: string, locationId?: string) {
    ...
    const params: any[] = [teamId, WORKSPACE_ID];
    let location = '';
    if (locationId) { params.push(locationId); location = `AND p.location_id=$${params.length}`; }
    const { rows } = await this.db.query(
      `SELECT p.id,p.user_id AS "userId",p.location_id AS "locationId",p.name,p.email,p.phone,p.title,p.availability,p.is_active AS "isActive",l.name AS "locationName"
        ...
        ORDER BY p.is_active DESC,p.name ASC`, params);
    return { items: rows };
  }

  async saveProvider(teamId: string, body: any, id?: string) {
    this.requireTeam(teamId);
    if (!String(body?.name || '').trim()) throw new BadRequestException('Provider name is required.');
    const params = [teamId, WORKSPACE_ID, body.userId || null, body.locationId || null, String(body.name).trim(), body.email || null, body.phone || null, body.title || null, JSON.stringify(body.availability || {}), body.isActive !== false];
    if (id) {
      const { rows } = await this.db.query(
        `UPDATE aesthetic_providers SET user_id=$3,location_id=$4,name=$5,email=$6,phone=$7,title=$8,availability=$9::jsonb,is_active=$10,updated_at=now()
          WHERE id=$11 AND team_id=$1 AND workspace_id=$2 RETURNING *`, [...params, id]);
    ...
    const { rows } = await this.db.query(
      `INSERT INTO aesthetic_providers(team_id,workspace_id,user_id,location_id,name,email,phone,title,availability,is_active)
       VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10) RETURNING *`, params);
    return rows[0];
  }
  ...
  async listTreatments(teamId: string) {
    ...
    const { rows } = await this.db.query(
      `SELECT id,name,category,description,duration_minutes AS "durationMinutes",price,rebooking_days AS "rebookingDays",booking_buffer_before AS "bufferBefore",booking_buffer_after AS "bufferAfter",is_active AS "isActive"
        ...
        ORDER BY is_active DESC,name ASC`, [teamId, WORKSPACE_ID]);
    return { items: rows.map((r: any) => ({ ...r, price: Number(r.price || 0) })) };
  }

  async saveTreatment(teamId: string, body: any, id?: string) {
    this.requireTeam(teamId);
    if (!String(body?.name || '').trim()) throw new BadRequestException('Treatment name is required.');
    const params = [teamId, WORKSPACE_ID, String(body.name).trim(), body.category || null, body.description || null, Math.max(5, Number(body.durationMinutes || 30)), Math.max(0, Number(body.price || 0)), body.rebookingDays == null || body.rebookingDays === '' ? null : Math.max(1, Number(body.rebookingDays)), Math.max(0, Number(body.bufferBefore || 0)), Math.max(0, Number(body.bufferAfter || 0)), body.isActive !== false];
    if (id) {
      const { rows } = await this.db.query(
        `UPDATE aesthetic_treatments SET name=$3,category=$4,description=$5,duration_minutes=$6,price=$7,rebooking_days=$8,booking_buffer_before=$9,booking_buffer_after=$10,is_active=$11,updated_at=now()
          WHERE id=$12 AND team_id=$1 AND workspace_id=$2 RETURNING *`, [...params, id]);
  ...
  async getConversionFlow(teamId: string) {
    const [locations, providers, treatments, setup] = await Promise.all([
      this.listLocations(teamId), this.listProviders(teamId), this.listTreatments(teamId), this.getWorkspaceConfig(teamId),
    ]);
    return { workspaceId: WORKSPACE_ID, locations: locations.items, providers: providers.items, treatments: treatments.items, pipelineConfig: setup.pipelineConfig || {} };
  }
  ...
  async getWorkspaceConfig(teamId: string) {
    ...
    const { rows } = await this.db.query(
      `SELECT business_profile AS "businessProfile",appointment_rules AS "appointmentRules",behavior,automations,pipeline_config AS "pipelineConfig",qualification_questions AS "qualificationQuestions",safety_rules AS "safetyRules",faqs,business_profile_completed AS "businessProfileCompleted",appointment_rules_configured AS "appointmentRulesConfigured",behavior_configured AS "behaviorConfigured",automations_configured AS "automationsConfigured",tested,launched,paused
         FROM workspace_ai_setup_configs WHERE team_id=$1 AND workspace_id=$2 LIMIT 1`, [teamId, WORKSPACE_ID]);
    return rows[0];
  }
  ...
  async ensureWorkspaceConfig(teamId: string) {
    ...
    await this.db.query(
      `INSERT INTO workspace_ai_setup_configs(team_id,workspace_id) VALUES($1,$2)
       ON CONFLICT(team_id,workspace_id) DO NOTHING`, [teamId, WORKSPACE_ID]);
  }
}
