...
export class AestheticWellnessController {
  ...
  dashboard(
    ...
    return this.service.getDashboard(this.teamId(user), { range, locationId, providerId });
  }
  ...
  savePipeline(@CurrentUser() user: any, @Body() body: any) {
    return this.service.savePipelineConfig(this.teamId(user), body?.pipelineConfig || body || {});
  }
  ...
  @Get('providers')
  providers(@CurrentUser() user: any, @Query('location_id') locationId?: string) {
  ...
  @Patch('providers/:id')
  updateProvider(@CurrentUser() user: any, @Param('id') id: string, @Body() body: any) {
  ...
  @Patch('treatments/:id')
  updateTreatment(@CurrentUser() user: any, @Param('id') id: string, @Body() body: any) {
