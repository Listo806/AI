...
export class SetupService {
 ...
 private async workspace(teamId:string, requested?:string){
   if(requested){ const q=await this.db.query(`SELECT workspace_id FROM workspace_entitlements WHERE team_id=$1 AND workspace_id=$2 AND status='active' LIMIT 1`,[teamId,requested]); if(!q.rows[0]) throw new ForbiddenException('Workspace access denied'); return requested; }
   const q=await this.db.query(`SELECT workspace_id FROM workspace_entitlements WHERE team_id=$1 AND status='active' ORDER BY created_at ASC LIMIT 1`,[teamId]);
   return q.rows[0]?.workspace_id || 'default';
 }
 ...
 async get(u:any,requested?:string){
   ...
   const r=await this.db.query(
     `SELECT * FROM customer_setup_configs WHERE team_id=$1 AND workspace_id=$2`,
     [teamId,ws]
   );
