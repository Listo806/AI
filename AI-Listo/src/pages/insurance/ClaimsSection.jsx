...
function toClaimRow(c) {
  return {
    uuid: c.id,
    number: c.claimNumber || c.id,
    policy: c.policyNumber || "-",
    holder: c.policyHolder || c.contactName || "",
    type: c.claimType || "-",
    filed: formatDate(c.filedDate),
    claimed: formatMoney(c.amountClaimed),
    approved: formatMoney(c.amountApproved),
...
export default function ClaimsSection() {
  ...
  const [modalClaimId, setModalClaimId] = useState(null);
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      insuranceApi
        .listClaims({ search: search.trim() || undefined, page, limit })
        .then((res) => {
          if (cancelled) return;
  ...
  return (
    <section className="insurance-ws-policies">
      <div className="insurance-ws-section-head">
        <div>
          <h2>Claims</h2>
          <p>Track and manage claims against your policies</p>
        </div>
      ...
      <div className="insurance-ws-table-wrap">
        <table className="insurance-ws-table">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={10} style={EMPTY_CELL_STYLE}>
                  Loading claims...
                </td>
              </tr>
            ) : error ? (
              ...
              <tr>
                <td colSpan={10} style={EMPTY_CELL_STYLE}>
                  {search.trim()
                    ...
                    : "No claims yet. Create a claim against a policy to get started."}
                </td>
              </tr>
            ) : (
              claims.map((claim) => (
                <tr key={claim.uuid}>
                  ...
                  <td className="policy-id">{claim.number}</td>
                  <td>
                    <div className="insurance-ws-two-line">
                      <strong>{claim.policy}</strong>
                      {claim.holder && <span>{claim.holder}</span>}
                    </div>
                  </td>
                  <td>{claim.type}</td>
                  <td>{claim.filed}</td>
                  ...
                  <td>{claim.paid}</td>
                  <td>
                    <span className="iw-pill">{claim.status}</span>
                  </td>
                  <td>
                    <div className="insurance-ws-row-actions">
                      <Eye
                        ...
                        onClick={() => openView(claim.uuid)}
                      />
                      <Pencil
                        size={14}
                        style={{ cursor: "pointer" }}
                        onClick={() => openEdit(claim.uuid)}
                      />
                      <MoreVertical
                        size={14}
                        style={{ cursor: "pointer" }}
      ...
      <div className="insurance-ws-pagination">
        <span>
          {total === 0
            ? "No claims"
            : `Showing ${from} to ${to} of ${total.toLocaleString("en-US")} claims`}
        </span>
        <div>
          <button
            disabled={page <= 1}
