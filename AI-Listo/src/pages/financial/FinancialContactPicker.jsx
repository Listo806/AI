...
// Search + select an existing Cortexa contact to attach a financial client to
// (reuse, not a second customer database). value: { id, name } | null. Reuses the
// iw-contact-* styles from the shared work modal.
export default function FinancialContactPicker({ value, onChange }) {
  ...
  if (value && value.id) {
    return (
      <div className="iw-contact-selected">
        ...
        <button type="button" onClick={() => onChange(null)} aria-label="Remove linked contact">
          <X size={14} />
        </button>
      </div>
    );
