...
/*
 * E-COMMERCE DATA BOUNDARY
 * ---------------------------------------------------------
 * This page intentionally keeps the exact AdminCustomers UX,
 * but it MUST NOT use ../../api/platformApi or Cortexa Admin customer data.
 *
 * Replace these local placeholder functions with tenant-scoped /api/ecommerce/*
...
function ECommerceSubscriptionsUI() {
  ...
  return (
    <div className="cxc-page">
      ...
      <div className="cxc-header">
        <div>
          <h1 className="cxc-title">E-commerce Subscriptions Workspace</h1>
      ...
      <div className="cxc-panel">
        ...
        <div className="cxc-pagination">
          <div className="cxc-page-info">
            {total ? `Showing ${(page - 1) * rowsPerPage + 1} to ${Math.min(page * rowsPerPage, total)} of ${total.toLocaleString()} subscriptions` : "No subscriptions"}
          </div>
...
function ECommerceLockedPreview() {
  const handleAddToPlan = () => {
    const addon = FEATURE_TO_ADDON?.[ECOMMERCE_WORKSPACE_FEATURE];
  ...
  return (
    <div className="cxc-page ecw-locked-page">
      <div className="ecw-locked-card">
        <div className="ecw-locked-icon">
          <Gift size={22} />
        </div>

        <div className="ecw-locked-copy">
          <span className="ecw-locked-kicker">CORTEXA WORKSPACE</span>
          <h1>E-Commerce Workspace</h1>
          <p>
            Manage customers, subscriptions, products, orders, inventory, billing,
            and revenue in one connected workspace.
          </p>
        </div>

        <div className="ecw-locked-features">
          <span><UserCheck size={15} /> Customer & subscription management</span>
          <span><CreditCard size={15} /> Billing & payment visibility</span>
          <span><Receipt size={15} /> Orders & revenue tracking</span>
          <span><ActivityIcon size={15} /> Workspace analytics</span>
        </div>

        <div className="ecw-locked-footer">
          <div>
            <small>Starting at</small>
            <strong>$97 <em>/month</em></strong>
          </div>
...
export default function ECommerceWorkspace() {
  ...
  const isInternalUser = INTERNAL_WORKSPACE_ROLES.has(role);

  // Internal Cortexa staff can QA the customer-side UX.
  // Regular customers only see the full workspace after purchasing/activating it.
  const hasWorkspaceAccess =
    isInternalUser ||
    !!plan?.hasFeature?.(ECOMMERCE_WORKSPACE_FEATURE);
  ...
  if (!hasWorkspaceAccess) {
    return <ECommerceLockedPreview />;
  }

  return <ECommerceSubscriptionsUI />;
}