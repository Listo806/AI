...
export default function TestLaunchSetup(){
 ...
 const run=async()=>{if(!channel)return;setBusy(true);try{const r=await setupApi.test({...f,channel}, data?.workspace_id);setResult(r?.result||null);await load()}finally{setBusy(false)}};
 ...
 return <main className="setup-shell pass4-page">
  ...
  <header className="p4-header"><span className="p4-title-icon"><Play/></span><div><h1>{t.title}</h1><p>{t.subtitle}</p></div><span className="p4-ready"><CheckCircle2/>{t.ready}</span><button className="p4-outline">{t.save}</button><button className="p4-outline">{t.summary}</button></header>
  ...
  <div className="p4-layout">
   ...
   <section className="p4-main">
    ...
    <div className="p4-customer"><label><span>{t.customer}</span><div><UserRound/><input value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></div></label><label><span>&nbsp;</span><div><Phone/><input value={f.phone} onChange={e=>setF({...f,phone:e.target.value})}/></div></label><label><span>&nbsp;</span><div><Mail/><input value={f.email} onChange={e=>setF({...f,email:e.target.value})}/></div></label><button disabled={busy||!channel} onClick={run}><Play/>{busy?"Testing…":t.start}</button></div>
    <div className="p4-chat">
      ...
      <div className="p4-compose"><Paperclip/><input disabled placeholder={t.type}/><Send/></div>
     </div>
   ...
   <aside className="p4-right">
    ...
    <section className="p4-card p4-crm"><header><h2>{t.crm}</h2><button>{t.view}</button></header><dl><dt>Lead:</dt><dd>{latest?.customerName||latest?.name||"—"}</dd><dt>Source:</dt><dd>{latest?.source||"—"}</dd><dt>Interest:</dt><dd>{latest?.interest||"—"}</dd><dt>Pipeline:</dt><dd>{latest?.pipeline||latest?.pipelineId||"—"}</dd><dt>Stage:</dt><dd>{latest?.stage||latest?.stageId||"—"}</dd><dt>Owner:</dt><dd>{latest?.owner||"—"}</dd></dl></section>
  ...
  <footer className="p4-footer"><button className="back" onClick={()=>navigate("/dashboard/ai-cortexa-setup/customer-entry-points?section=conversion")}><ArrowLeft/>{t.back}</button><span><CheckCircle2/>{state||t.saved}</span><div><button className="reset" onClick={()=>setResult(null)}>{t.reset}</button><button className="finish" disabled={!testPassed} onClick={load}>{t.finish}<ArrowRight/></button></div></footer>
 </main>
}
